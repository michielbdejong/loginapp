...
unhosted = new function() {
	...
	var sendPost = function(post, storageNode, resource) {//this function implements synchronous AJAX to a storageNode
		...
		xmlhttp.open("POST","http://unhosted."+storageNode+"/"+resource+"/",false);//TODO: make this https
	...
	var createPub = function(user, storageNode, creationToken) {
		var bnSeskey, bnWriteKey, bnReadKey, cmd, ret, guid;
		key = RSAGenerate();
		...
		ret = sendPost("protocol=UJ/0.2&action=ACCT.CREATE&user="+user+"&subPass="+key.subPass+"&pubPass="+key.pubPass+"&creationToken="+creationToken, storageNode);
		return [key, ret];
	}
	//public:
...
//		}
	...
	this.importWallet = function(guid, walletHost, walletPass) {//execute a getwallet command
		var ret = sendPost("protocol=KeyWallet/0.1&action=GET&email="+guid+"&password="+walletPass, walletHost, "wallet");
		this.importPub(JSON.parse(ret), guid);
		var ret = sendPost("protocol=UJ/0.2&action=ACCT.GETSTATE&user="+keys[guid].user+"&pubPass="+keys[guid].pubPass, keys[guid].storageNode);
		switch(ret) {
		case '0':
	...
	this.registerWithCaptcha = function(guid, captchaSolution) {
		...
		if(parts.length != 2) {
			...
			resParts = createPub(parts[0], parts[1], captchaSolution);
			this.importPub(resParts[0], guid);
		}
		return resParts[1];
	}
}
